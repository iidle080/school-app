...
export function ParentProvider({ children: kids }: { children: ReactNode }) {
  ...
  // Restore / set default selected child
  useEffect(() => {
  ...
  const selectedChildClass = selectedChild?.class_id
    ? classes.find((c) => c.id === selectedChild.class_id) ?? null
    : null;

  return (
    <ParentContext.Provider
      value={{ children, classes, selectedChild, selectedChildClass, loading, selectChild, refresh: load }}
    >
      {kids}
    </ParentContext.Provider>
  );
