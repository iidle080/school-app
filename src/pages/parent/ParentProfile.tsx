...
export function ParentProfile() {
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const path = `${profile.school_id}/parents/${profile.id}/avatar.${ext}`;
          ...
          <Card className="text-center">
            ...
            <div className="mt-4 grid grid-cols-2 gap-3 border-t border-slate-100 pt-4 dark:border-slate-800">
              ...
              <div>
                ...
                <p className="truncate text-xs font-mono text-ink-muted" title={profile.user_id}>
  ...
  return (
    <div className="flex items-start gap-3 rounded-xl border border-slate-100 p-3 dark:border-slate-800">
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="break-words text-sm text-ink dark:text-slate-100">{value}</p>
      </div>
    </div>
  );
}
