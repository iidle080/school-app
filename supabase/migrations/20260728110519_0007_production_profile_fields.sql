...
Existing columns already cover: photo_url, gender, date_of_birth,
emergency_contact_name, emergency_contact_phone, medical_notes.

### 3. academic_years — archive support
Add column: archived (boolean, default false).
Allows school admin to archive old years without deleting them.

### 4. exam_sessions — ensure created_by tracks the admin
No schema change needed (created_by already exists).

### 5. exam_marks — add position and remarks columns
Add: position (integer, nullable), remarks (text, nullable).
These store computed class position and teacher remarks per student per exam.
